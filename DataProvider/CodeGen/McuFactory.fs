...
let getOptMaskField (name : string) fields =
    fields
    |> Seq.tryPick (function (name2, Value.Mask n) when name = name2 -> Some n | _ -> None)
...
let private mkAsHasEntity typeName path (state : (string * Value) list ref) iconLC subtitleLC formation =
    let baseImpl = mkAsBase typeName path state iconLC subtitleLC
    {
        new HasEntity with
            ...
            member this.WMMask
                with get() =
                    !state |> getOptMaskField "WMMask"
                and set mask =
                    state := !state |> setOptField ("WMMask", mask |> Option.map Value.Mask)
